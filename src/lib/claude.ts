...
- 목표 금액까지의 현실적 궤적 분석
- 인과관계 체인 포함 (예: "금리 동결 → 유동성 유지 → 성장주 우호적 → ETF 리스크 완화")
- 확신도를 [High/Medium/Low]로 표시. 단정 금지 ("~하세요" → "~를 고려할 수 있습니다")
- 반대 논거를 반드시 1줄 포함하여 양면 시각 제공
- 전문 용어 첫 등장 시 괄호 안에 간단한 설명 추가 (예: "샤프비율(위험 대비 수익 효율)")
- "오늘의 투자 한줄 지식" 1개를 반드시 포함 — 투자 초보자가 하나라도 배워가는 교육적 가치
...
반드시 아래 JSON 형식으로만 응답해:
{
  ...
  "finance_coaching": {
    ...
    "conviction": "high|medium|low (이 조언에 대한 확신도)",
    "counter_argument": "반대 논거 1줄 (예: 다만 ~한 경우에는 ~할 수 있습니다)",
    "causal_chain": "인과관계 체인 (예: 금리 동결 → 유동성 유지 → 성장주 우호적)",
    "daily_knowledge": "오늘의 투자 한줄 지식 (투자 초보자에게 유익한 개념 하나)"
  },
  "cross_insight": {
